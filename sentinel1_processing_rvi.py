...
Script unificato che elabora le scene Sentinel-1 in due fasi consecutive:
  ...
  Fase 1 — Processing SNAP GPT (.SAFE -> .dim):
    ...
    Catena di processing (definita nel grafo XML esterno):
      ...
      7. Output BEAM-DIMAP (.dim)   -> salva con bande Sigma0_VV e Sigma0_VH
  ...
  Fase 2 — Calcolo RVI (.dim -> .tif):
    ...
    Modalita' di calcolo:
      Default:              Solo scene closest S1, poligoni FALSE.
                            Produce sostituzioni_rvi.csv.
      --all_polygons:       Solo scene closest S1, tutti i poligoni AOI.
                            Non produce sostituzioni_rvi.csv.
      --ignore_inventory:   Tutte le scene .dim, tutti i poligoni AOI.
                            Non produce sostituzioni_rvi.csv.
                            Nota: la combinazione tutte le scene + poligoni FALSE
                            non e' possibile senza inventario (non si saprebbe
                            quali poligoni FALSE associare a ogni scena S1).
...
NOTE:
  ...
  - Righe con closest_delta_days mancante (NaT) nell'inventario vengono saltate.

STRUTTURA CARTELLE ATTESA:
  output_sentinel1/full_scenes/
    S1A_IW_GRDH_....SAFE/        <- input fase 1 (non necessario con --only_rvi)
  inventory_output/
...
USO:
  ...
  # Entrambe le fasi (comportamento normale):
...
Opzioni principali:
  ...
  --all_polygons                                          RVI su tutti i poligoni (default: solo poligoni FALSE)
  --only_rvi                                              Salta Fase 1 (SNAP), esegue solo Fase 2 (RVI)
  --ignore_inventory                                      Ignora inventario S1: processa tutti i .dim in snap_dir

Requisiti:
  pip install geopandas shapely rasterio numpy pandas
  SNAP installato con GPT accessibile da PATH (o specificare --gpt)
  (SNAP non necessario se si usa --only_rvi)
"""
...
def build_scene_to_dates(inventory: pd.DataFrame) -> dict:
    ...
    for _, row in inventory.iterrows():
        ...
        if stem not in scene_map:
            scene_map[stem] = []
        if pd.notna(row["closest_delta_days"]):
...
def run_rvi(
    ...
    # Carica qualita' S2 e inventario (solo se non si ignora l'inventario)
    scene_to_dates = {}
    df_quality     = None
    ...
    if not ignore_inventory:
        ...
        log.info("--ignore_inventory attivo: controllo inventario saltato, processate tutte le scene .dim.")
    ...
    if workers == 1:
        for i, dim_path in enumerate(dim_files, start=1):
            if ignore_inventory:
                # Ignora inventario: processa tutte le scene, tutti i poligoni
                active, sostituzioni_scene = None, []
            elif all_polygons:
                # Usa inventario per filtrare le scene, ma tutti i poligoni
                in_inventory = dim_path.stem in scene_to_dates
                if not in_inventory:
                    log.info(f"[{i}/{total}] SKIP RVI (non in inventario): {dim_path.name}")
                    results.append({"dim": dim_path.name, "date": parse_date_from_name(dim_path.name), "status": "skipped", "elapsed_s": 0, "size_mb": 0})
                    continue
                active, sostituzioni_scene = None, []
            else:
        ...
        def process_one_rvi(i, dim_path):
            if ignore_inventory:
                ...
                in_inventory = dim_path.stem in scene_to_dates
                if not in_inventory:
                    skipped = {"dim": dim_path.name, "date": parse_date_from_name(dim_path.name), "status": "skipped", "elapsed_s": 0, "size_mb": 0}
                    return skipped, []
                active, sostituzioni_scene = None, []
            else:
                active, sostituzioni_scene = get_false_polygons(
    ...
    if not all_polygons and not ignore_inventory and all_sostituzioni:
...
def run(
    ...
    if not only_rvi:
        ...
        log.info("--only_rvi attivo: processing SNAP saltato.")

    # ── Fase 2: Calcolo RVI ──
    run_rvi(snap_dir, rvi_dir, gdf_aoi, csv_path, inventory_path, nodata, workers, all_polygons, ignore_inventory)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--only_rvi",    action="store_true",         help="Salta il processing SNAP (Fase 1) e calcola solo l'RVI (Fase 2)")
    parser.add_argument("--ignore_inventory", action="store_true",      help="Ignora inventario S1: processa tutti i .dim trovati in snap_dir")
    ...
    run(
        ...
        only_rvi          = args.only_rvi,
        ignore_inventory  = args.ignore_inventory,
    )
